...
def order_stock(products: pd.DataFrame, prediction_data: pd.Series) -> pd.DataFrame:
    for _, row in products.iterrows():
        ...
        print(prediction_data.loc[prediction_data['name'] == row['name'], 'sales'].astype(int))
...
def generate_sales_data(product_name: str, sales: pd.DataFrame) -> pd.DataFrame:
    ...
    sales_data = np.random.randint(0, 50, 14)
    df_row = [product_name]
    df_row.extend(sales_data.tolist())
    print(df_row)
    sales.loc[len(sales), :] = df_row
    print(sales)
    return sales
...
def main() -> None:
    ...
    while True:
        ...
        if user_input == 0:
            ...
            products, sales = add_product(products, sales)
        elif user_input == 3:
            exit_store(products, sales)
            break
        else:
