# Exercise 9
# If you ran the previous exercise a few times, you may have noticed a bug where
# after adding a product, and then re-running the store you are unable to run the
# store. This is because the number of rows in the sales dataframe does not match
# the number of rows in the products dataframe. To fix that we must implement a new
# function that generates random sales data and appends it to the sales dataframe
# for every new product.

# Task 1: Implement the generate_sales_data function, following the specification
#   provided in the function body.

# Task 2: This mini-project is now complete! Feel free to play around with the
#   example solution provided and add some enhancements of your own if you can
#   think of any!
...
def add_product(products: pd.DataFrame, sales: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    updated_sales = generate_sales_data(name, sales)

    return products.reset_index(), updated_sales


def generate_sales_data(product_name: str, sales: pd.DataFrame) -> pd.DataFrame:
    # This function should insert a new row to the "sales" dataframe (read from sales.csv)
    # where the name is taken from the product_name argument and the sales are 14 randomly
    # generated numbers in the range (0, 50).
...
def main() -> None:
    ...
    while True:
        ...
        if user_input == 0:
            ...
            products, predicted_sales = add_product(products, predicted_sales)
        elif user_input == 3:
