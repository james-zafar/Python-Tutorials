...
def is_prime(number: int) -> bool:
    # Note we do not want to include 2 in this check
    if number < 2:
        return False
    # We also need to add a new check to return False if the number is even
    # as our range() function starts at 3 and only checks odd numbers
    if number % 2 == 0:
        return False
