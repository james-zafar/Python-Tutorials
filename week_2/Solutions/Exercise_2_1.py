...
def calculate_simple_interest(starting_amount: Union[int, float], interest_rate: float, years: int) -> float:
    if interest_rate > max_simple_interest:
        interest_rate = max_simple_interest
    return round(starting_amount + (starting_amount * interest_rate * years), 2)


def calculate_compound_interest(starting_amount: Union[int, float], interest_rate: float, years: int) -> float:
    if interest_rate > max_compound_interest:
        interest_rate = max_compound_interest
    return round(starting_amount * ((1 + interest_rate) ** years), 2)
