...
#  - Add a new DATABASE_FILE constant in __init__
#  - Change init_db to check if the DATABASE_FILE exists, and if so read it
#  - Change save_db to write to the file specified in the DATABASE_FILE constant.
# For exercise 2:
...
class TicTacToe:
    ...
    def get_ai_move(self) -> tuple[int, int]:
        ...
        # Other ways to turn a 2-d list into a 1-d list that are valid include:
        # Using functools
        # functools.reduce(lambda x, y: x+y, empty_cells)
        # functools combined with operator.add to simplify lambda
        # functools.reduce(operator.add, empty_cells)
        # Using itertools
